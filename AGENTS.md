...
9. Stack bloccato: SvelteKit 2, TypeScript, Tailwind, DaisyUI **tema custom Slate/Peach** (non `fantasy`), Drizzle + better-sqlite3, SQLite WAL + foreign keys, Better Auth email/password cookie httpOnly, i18n it/fr/en, vite-plugin-pwa. Email transazionali via SMTP2GO (reset e cambio password). Form actions + load come primario.
